...
class SEIR_PU_MOD(SEIR):
    def __init__(self, lockdown_R0=2.2, T_inf = 4.5, T_inc=5, T_recov_fatal=32,
                 P_fatal=0.2, T_recov=14, N=1e7, d=1.0, psi=1.00, beta=0.1, starting_date='2020-03-09', 
                 observed_values=None, E_hosp_ratio=0.5, I_hosp_ratio=0.5, I_U_hosp_ratio=0.5, Pu_pop_ratio=1.2,**kwargs):
        """
        ...
        """
        STATES = ['S', 'E', 'I', 'I_U', 'P_U', 'R_severe', 'R_fatal', 'C', 'D']
        ...
        self.I_hosp_ratio = I_hosp_ratio
        self.I_U_hosp_ratio = I_U_hosp_ratio
        self.state_init_values['I_U'] = observed_values['I_U']/N if self.I_U_hosp_ratio=='true' else self.I_U_hosp_ratio * observed_values['active']/N
        # self.state_init_values['I_D'] = observed_values['I_D']/N if self.I_D_hosp_ratio=='true' else self.I_D_hosp_ratio * observed_values['active']/N
    ...
    def get_derivative(self, t, y):
        ...
        for i, _ in enumerate(y):
            y[i] = max(y[i], 0)
        S, E, I, I_U, P_U, R_severe, R_fatal, C, D = y
        ...
        dydt[0] = - (I + I_U) * S * self.beta  # S
        dydt[1] = (I + I_U) * S * self.beta - (E/ self.T_inc)  # E
        dydt[2] = (1 / self.T_inc)*E - (1 - self.d) * I / self.T_inf - I*self.d / self.T_inf  # I
        dydt[3] = (1 - self.d) * I / self.T_inf - I_U / self.T_recov  # I_U
        dydt[4] = I_U / self.T_recov  # P_U
        dydt[5] = (self.P_severe*I*self.d) / self.T_inf - R_severe/self.T_recov #R_severe
        dydt[6] = (self.P_fatal*I*self.d) / self.T_inf - R_fatal/self.T_recov_fatal # R_fatal
