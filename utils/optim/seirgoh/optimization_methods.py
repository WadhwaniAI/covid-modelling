...
def tpe_opt(num_int, objective='qald', total_resource=80, day0=10, days=500, capacity=np.array([0.1]), sir_init=None):
	min_duration = 10
	if(num_int==1):
...
def tpe_grid(num_int, min_params, objective='qald', total_resource=80, days=500, iters=3000, capacity=np.array([0.1]), sir_init=None):
	min_duration = 10
	window_start = 30
	if(num_int==1):
	...
	if(num_int==3):
		start = max(int(min_params['start_array'][0] - window_start/2),0)
		end = int(min_params['start_array'][0] + window_start/2)
		start_val_0 = [start for start in range(start, end)]
		start = int(min_params['start_array'][1] - window_start/2)
		end = int(min_params['start_array'][1] + window_start/2)
