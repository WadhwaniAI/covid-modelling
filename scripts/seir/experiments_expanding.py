...
from main.ihme_seir.utils import get_seir_pointwise_loss_dict, get_seir_pointwise_loss, read_config, read_params_file, \
    create_pointwise_loss_csv, create_output_folder, supported_models
...
def run_experiments(config_path, output_folder, num):
    ...
    train_period = config['train_period'] + num
    ...
    if verbose:
        ...
        print("Training ends on:", start_date + timedelta(train_period))
    ...
    root_folder = f'{region_name}/{output_folder}_expanding/{str(num)}'
    if verbose:
...
def outputs(path, start=0, end=0):
    ...
    for model in models:
        val_loss_dict[model] = get_seir_pointwise_loss_dict(path, f'{model}_pointwise_val_loss.csv', start=start,
                                                            end=end)
        for compartment in compartments:
            val_loss = get_seir_pointwise_loss(val_loss_dict[model], compartment=compartment, loss_fn='ape')
            create_pointwise_loss_csv(path, val_loss, val_period, model, compartment, start, end)
