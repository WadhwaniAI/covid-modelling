...
if __name__ == '__main__':
    ...
    if First_time:
        ...
        with open('Result_SIR_refits={}_episodes={}_episode_length={}.pickle'.format(num_refits,num_episodes,episode_length), 'wb') as f:
                    pickle.dump([env,episodes], f)
    else:       
        with open('Result_SIR_refits={}_episodes={}_episode_length={}.pickle'.format(num_refits,num_episodes,episode_length), 'rb') as f:
            X = pickle.load(f)
        sys.path.pop(-1)
        env=X[0]
        episodes=X[1]
